...
class PaymentMethods extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Only put the controller if we don't have data passed in (e.g. vendor's own profile)
    final controller = (methods == null && extraDetails == null) 
        ? Get.put(GroomViewProfileController()) 
        : null;
    ...
    return Scaffold(
      ...
      body: Builder(builder: (context) {
        // Use Obx if we're using the controller, otherwise just use the passed data
        if (controller != null) {
          return Obx(() {
            ...
            final m = controller.paymentMethods;
            final e = controller.vendorData['otherPaymentDetails']?.toString() ?? '';
            return _buildContent(m, e);
          });
        } else {
          return _buildContent(methods ?? [], extraDetails ?? '');
        }
      }),
    );
  }

  Widget _buildContent(List<String> methods, String extraDetails) {
