...
class ApplicationCompleteFarrierScreen extends StatefulWidget {
  final bool isLastForm;
  final VoidCallback onContinue;

  const ApplicationCompleteFarrierScreen({
    super.key,
  ...
  @override
  State<ApplicationCompleteFarrierScreen> createState() =>
      _ApplicationCompleteFarrierScreenState();
}

class _ApplicationCompleteFarrierScreenState
    extends State<ApplicationCompleteFarrierScreen> {
  ...
  final Set<String> _selectedScopeOfWork = {};
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Farrier Application'),
        centerTitle: true,
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'This application confirms your professionalism, experience, and scope of operations as a farrier.',
            ...
            // 2. Experience
            _header('Years of Professional Farrier Experience', required: true),
            ...
            // 3. Certifications
            _header('Relevant Certifications'),
            ...['AFA', 'BWFA'].map((c) {
              return CheckboxListTile(
                title: Text(c),
                value: _selectedCertifications.contains(c),
                activeColor: AppColors.deepNavy,
                ...
                onChanged: (val) {
                  setState(() {
                    if (val == true) {
                      _selectedCertifications.add(c);
                    } else {
                      _selectedCertifications.remove(c);
                    }
                  });
                },
              );
            ...
            Row(
              children: [
                Checkbox(
                  value: _selectedCertifications.contains('Other'),
                  activeColor: AppColors.deepNavy,
                  onChanged: (val) {
                    setState(() {
                      if (val == true) {
                        _selectedCertifications.add('Other');
                      } else {
                        _selectedCertifications.remove('Other');
                      }
                    });
                  },
                ),
                ...
                if (_selectedCertifications.contains('Other'))
                  Expanded(
                    child: TextField(
            ...
            // 4. Scope of Work
            _header('Scope of Work'),
            ...[
              'Routine trimming/shoeing',
              'Performance horses',
              'Corrective / therapeutic work',
              'Glue-on / specialty shoes',
              'Vet-coordinated cases',
              'Barefoot Specialist',
            ].map((scope) {
              return CheckboxListTile(
                title: Text(scope),
                value: _selectedScopeOfWork.contains(scope),
                activeColor: AppColors.deepNavy,
                controlAffinity: ListTileControlAffinity.leading,
                contentPadding: EdgeInsets.zero,
                onChanged: (val) {
                  setState(() {
                    if (val == true) {
                      _selectedScopeOfWork.add(scope);
                    } else {
                      _selectedScopeOfWork.remove(scope);
                    }
                  });
                },
              );
            ...
            _buildAck(
              'I carry professional liability insurance or understand it may be required for certain bookings.',
              _ackInsurance,
              (v) => setState(() => _ackInsurance = v ?? false),
            ),
            _buildAck(
