...
class GoogleApiController extends GetxController {
  ...
  String _formatAddress(String description) {
    ...
    // Show only city, state, country (last 3 components)
    if (parts.length > 3) {
    ...
    for (int i = 0; i < parts.length; i++) {
      ...
      } else if (upperPart == 'USA' || upperPart == 'US') {
        parts[i] = 'USA';
      }
    }
    return parts.join(', ');
  ...
  Future<void> searchGooglePlaces(
    String query, {
    String? location,
    int? radius,
  }) async {
    ...
    try {
      String url =
          "https://maps.googleapis.com/maps/api/place/autocomplete/json?input=$query&key=$googleApiKey&components=country:us";
      ...
      if (response.statusCode == 200) {
        ...
        final List<Map<String, String>> uniqueSuggestions = [];
        final Set<String> seenNames = {};

        for (var p in predictions) {
          String description = p['description']?.toString() ?? '';
          String formattedName = _formatAddress(description);

          if (!seenNames.contains(formattedName)) {
        ...
        googleSuggestions.assignAll(uniqueSuggestions);
      }
