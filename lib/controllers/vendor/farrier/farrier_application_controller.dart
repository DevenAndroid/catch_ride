...
class FarrierApplicationController extends GetxController {
  ...
  Future<void> fetchDynamicTags() async {
    ...
    try {
      final Response response = await apiService.getRequest('/system-config/tag-types/with-values?category=Farrier');
      if (response.statusCode == 200 && response.body['success'] == true) {
        final List types = response.body['data'];
        
        // Populate Relevant Certifications
        final certType = types.firstWhereOrNull((t) => t['name'] == 'Relevant Certifications');
        if (certType != null) {
          certificationOptions.assignAll(List<String>.from(certType['values'].map((v) => v['name'])));
        }

        // Populate Scope of Work
        final scopeType = types.firstWhereOrNull((t) => t['name'] == 'Scope of Work');
        if (scopeType != null) {
          scopeOfWorkOptions.assignAll(List<String>.from(scopeType['values'].map((v) => v['name'])));
        }
  ...
  void _setFallbackOptions() {
    if (certificationOptions.isEmpty) {
      certificationOptions.assignAll(['AFA', 'BWFA', 'Other']);
    }
    if (scopeOfWorkOptions.isEmpty) {
      scopeOfWorkOptions.assignAll([
        'Routine trimming/shoeing',
        'Corrective / therapeutic work',
        'Glue-on / specialty shoes',
        'Barefoot Specialist',
  ...
  Future<String?> _uploadPhoto(File file) async {
    try {
      ...
      final response = await apiService.postRequest('/upload?type=farrier', formData);
      if (response.statusCode == 200 && response.body['success'] == true) {
  ...
  Future<void> submitApplication() async {
    ...
    if (selectedScopeOfWork.isEmpty) {
      Get.snackbar('Missing Info', 'Please select at least one scope of work', backgroundColor: AppColors.accentRed, colorText: AppColors.cardColor);
      return;
    }
    ...
    try {
      ...
      final applicationData = {
        ...
        'relevantCertifications': selectedCertifications.toList(),
